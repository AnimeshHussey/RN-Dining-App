...
export const menuReducer = (state = INITIAL_STATE, action) => {
    ...
    switch (action.type) {
        ...
        case ReduxActions.SET_SELECTED_ITEMS:
            ...
            return Object.assign([], state, { selectedCategory: tempSelectedMenu, selectedIndex: action.selectedIndex })

        case ReduxActions.GOT_ALL_ITEMS:
            return Object.assign({}, state, { itemList: action.response })

        case ReduxActions.FAILED_TO_GET_ITEMS:
            return Object.assign({}, state);

        case ReduxActions.IS_ORDER_MODAL_OPEN:
            return Object.assign([], state, { isOrderModalOpen: action.isOrderModalOpen });

        case ReduxActions.SET_ORDERS:
            return Object.assign([], state, { Orders: action.selectedOrders });
    
        case ReduxActions.SELECTED_ITEM_FOR_REMARKS:
            return Object.assign([], state, { selectedItemforRemarks: action.ItemID });
            ...
            if (state.searchedVal && state.searchedArr.length > 0) {
                ...
                if (tempOrders.length === 0) {
                    ...
                    tempOrders.map((each, index) => {
                        ...
                        if (tempOrders[index].quantity === 0) { tempOrders.splice(index, 1) }
                    })
                    if (tempState === false) {
                        let temp = Object.assign({}, tempItemlist[dummySelIndex].Items[dummyItemIndex]);
                        temp.CategoryIndex = dummySelIndex;
                ...
                return (Object.assign({}, state, {Orders: tempOrders, selectedCategory: tempSearchedArr[state.selectedIndex].Items, itemList: tempItemlist, searchedArr: tempSearchedArr }))
            }
            else {
                if (action.toAdd) {
                ...
                return (Object.assign({}, state, { Orders: tempOrders, selectedCategory: tempItemlist[state.selectedIndex].Items, itemList: tempItemlist }))
            }
        ...
        case ReduxActions.CHANGE_QUANTITY_FROM_MODAL:
            ...
            if(state.searchedVal && state.searchedArr.length > 0){
                return (Object.assign({}, state, { Orders: tempOrders1, itemList: tempItemlist1, selectedCategory: state.searchedArr[state.selectedIndex].Items }))
            }

            else{
                return (Object.assign({}, state, { Orders: tempOrders1, itemList: tempItemlist1, selectedCategory: tempItemlist1[state.selectedIndex].Items }))
            }


        case ReduxActions.PLACING_ORDER:   
        return (Object.assign({}, state, { isOrderPlaced: "PLACING"}))
        ...
        case ReduxActions.SUCCESSFULLY_PLACED_ORDER:
            ...
            return (Object.assign({}, state, { isOrderPlaced: "SUCCESS", orderNumber: orderId, orderObj: tempOrdObj, Orders: [] }))

        case ReduxActions.FAILED_TO_PLACE_ORDER:
        return (Object.assign({}, state, { isOrderPlaced: "FAILED" }))

        case ReduxActions.RESET_ITEM_QUANTITY:
            let number = state.SubOrderNumber + 1;
            return Object.assign({}, state, { itemList: action.itemList, SubOrderNumber: number })

        case ReduxActions.SET_EXISTING_ORDER_ID:
            let OrdObj = Object.assign({}, state.orderObj);
            OrdObj.OrderID = action.OrderID;
            return Object.assign({}, state, { orderNumber: action.OrderID, SubOrderNumber: action.SubOrderNumber, orderObj: OrdObj })
        ...
        case ReduxActions.SET_ORDER_ID:
            return Object.assign({}, state, { orderNumber: action.response })

            case ReduxActions.IS_KEYBOARD_OPEN:
            return Object.assign({}, state, { isKeyBoardOpen: action.isKeyboardOpen })  
        ...
        case ReduxActions.UPDATE_REMARKS_LIST:
            ...
            allData.forEach((each, index) => {
                if (action.selectedRemarks.key === each.key) {
                    allData[index] = { "key": action.selectedRemarks.key, "value": !each.value }
                }
            })
            return Object.assign({}, state, { remarksList: allData })

        case ReduxActions.RESET_REMARKS:
            return Object.assign({}, state, { comment: INITIAL_STATE.comment })

        case ReduxActions.RESET_REMARKSLIST:
            var tempOut = [];
            state.remarksList.forEach((each, index) => {
                tempOut.push({ "key": each.key, "value": false })
            })
            return Object.assign({}, state, { remarksList: tempOut })

        case ReduxActions.COMMENT_REMOVED:
            return Object.assign({}, state, { Orders: action.newObj })

        case ReduxActions.SET_COMMENT:
            return Object.assign({}, state, { comment: action.remarks })

        case ReduxActions.SET_REMARKS:
            var tempOut = [];
            action.response.forEach((each, index) => {
                tempOut.push({ "key": each, "value": false })
            })
            return Object.assign([], state, { remarksList: tempOut });
        ...
        case ReduxActions.SET_SEARCH_RESULTS_MENU:
            return Object.assign({}, state, { searchedArr: action.searchedArr });

        case ReduxActions.IS_SEARCH_VISIBLE:
            return Object.assign({}, state, { searchVisable: action.searchVisable });
